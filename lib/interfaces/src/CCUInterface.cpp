...
void CCUInterface::handle_enqueue_acu_voltages_CAN_message() {
    ...
    msg.cell_group_voltage_0_ro = HYTECH_cell_group_voltage_0_ro_toS(_acu_all_data.cell_voltages[_curr_data.current_voltage_cell_id]); 
    msg.cell_group_voltage_1_ro = HYTECH_cell_group_voltage_1_ro_toS(_acu_all_data.cell_voltages[_curr_data.current_voltage_cell_id+1]); 
    msg.cell_group_voltage_2_ro = HYTECH_cell_group_voltage_2_ro_toS(_acu_all_data.cell_voltages[_curr_data.current_voltage_cell_id+2]); 

    if (_curr_data.current_voltage_group_chip_id % 2 == 0) {
        _curr_data.current_voltage_cell_group_id = (_curr_data.current_voltage_cell_group_id == _ccu_params.groups_per_ic_even - 1) ? 0 : _curr_data.current_voltage_cell_group_id+1;
    } else {
        _curr_data.current_voltage_cell_group_id = (_curr_data.current_voltage_cell_group_id == _ccu_params.groups_per_ic_odd - 1) ? 0 : _curr_data.current_voltage_cell_group_id+1;
    }
    if (_curr_data.current_voltage_cell_group_id == 0) {
        _curr_data.current_voltage_group_chip_id = (_curr_data.current_voltage_group_chip_id == (_ccu_params.num_chips - 1)) ? 0 : _curr_data.current_voltage_group_chip_id+1;
    }
    _curr_data.current_voltage_cell_id= (_curr_data.current_voltage_cell_id == _ccu_params.num_cells - _ccu_params.voltage_cells_per_group) ? 0 : _curr_data.current_voltage_cell_id+_ccu_params.voltage_cells_per_group;
...
void CCUInterface::handle_enqueue_acu_temps_CAN_message() {
    ...
    if (_curr_data.current_temp_group_id == 0) {
        _curr_data.current_temp_group_chip_id = (_curr_data.current_temp_group_chip_id == (_ccu_params.num_chips - 1)) ? 0 : _curr_data.current_temp_group_chip_id+1;
    }
    _curr_data.current_temp_cell_id = (_curr_data.current_temp_cell_id == (_ccu_params.num_celltemps - _ccu_params.temp_cells_per_group)) ? 0 : _curr_data.current_temp_cell_id+_ccu_params.temp_cells_per_group;
    ...
    current_board_temp_msg.temp_0_ro = HYTECH_temp_0_ro_toS(_acu_all_data.board_temps[_curr_data.current_temp_board_id]);
    _curr_data.current_temp_board_id = (_curr_data.current_temp_board_id == (_ccu_params.num_chips - 1)) ? 0 : _curr_data.current_temp_board_id+1;
