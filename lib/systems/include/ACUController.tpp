...
ACUController<num_cells, num_celltemps, num_boardtemps>::evaluate_accumulator(time_ms current_millis, const BMSCoreData_s<num_cells, num_celltemps, num_boardtemps> &input_state, float em_current, float em_voltage)
{   
...
float ACUController<num_cells, num_celltemps, num_boardtemps>::get_state_of_charge(float em_current, float em_voltage, uint32_t delta_time_ms, volt avg_cell_voltage)
{
    ...
    bool is_stabilized = (fabs(em_current) <= STABILIZED_CURRENT_THRESH && fabs(em_voltage) <= STABILIZED_VOLTAGE_THRESH);
    if (is_stabilized) {
