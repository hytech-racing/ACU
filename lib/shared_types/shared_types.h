...
enum ReadDataResultType_e { CELL_VOLTAGE, CELL_TEMPERATURE, BOARD_TEMPERATURE, NO_DATA };

// ----- leaf: per-chip group map (immutable) -----
template<std::size_t DATA_PER_GROUP>
struct ReadGroupResultMap {
    static constexpr std::size_t kNumGroups = ReadGroup_e::NUM_GROUPS;

    const std::array<size_t, kNumGroups> group_values;
    const std::array<std::array<ReadDataResultType_e, DATA_PER_GROUP>, kNumGroups> group_data_types;

    // small helper
    constexpr std::size_t data_per_group() const noexcept { return DATA_PER_GROUP; }
    constexpr std::size_t num_groups()     const noexcept { return kNumGroups;    }
};

// ----- chip (immutable) -----
template<std::size_t DATA_PER_GROUP>
struct Chips {
    const std::size_t addr_pin;
...
struct ChipSelect {
    ...
    const std::array<Chips<DATA_PER_GROUP>, NUM_CHIPS_PER_CS> chips;

    constexpr std::size_t num_chips() const noexcept { return NUM_CHIPS_PER_CS; }
};

// ----- full config (immutable) + constexpr derived views -----
template<std::size_t NUM_CS, std::size_t NUM_CHIPS_PER_CS, std::size_t DATA_PER_GROUP>
struct ChipSelectConfig {
    const std::array<ChipSelect<NUM_CHIPS_PER_CS, DATA_PER_GROUP>, NUM_CS> chip_selects;

    // Compile-time sizes
    static constexpr std::size_t kNumChipSelects   = NUM_CS;
    static constexpr std::size_t kNumChipsPerCS    = NUM_CHIPS_PER_CS;
    static constexpr std::size_t kDataPerGroup     = DATA_PER_GROUP;
    static constexpr std::size_t kTotalChips       = NUM_CS * NUM_CHIPS_PER_CS;

    // ---- Derived per-chip counts (voltage/temp/board), all constexpr ----
    constexpr std::array<std::size_t, kTotalChips> voltage_cells_per_chip() const {
        std::array<std::size_t, kTotalChips> out{}; // zero-inited
        for (std::size_t cs = 0; cs < kNumChipSelects; ++cs) {
            const auto& sel = chip_selects[cs];
            for (std::size_t c = 0; c < kNumChipsPerCS; ++c) {
                const auto& chip = sel.chips[c];
                const std::size_t flat = cs * kNumChipsPerCS + c;
                for (std::size_t g = 0; g < chip.read_map.num_groups(); ++g) {
                    if (!chip.read_map.is_group_on[g]) continue;
                    for (auto kind : chip.read_map.group_data_types[g]) {
                        if (kind == CELL_VOLTAGE) { ++out[flat]; }
                    }
                }
            }
        }
    ...
    constexpr std::array<std::size_t, kTotalChips> temp_cells_per_chip() const {
        std::array<std::size_t, kTotalChips> out{};
        for (std::size_t cs = 0; cs < kNumChipSelects; ++cs) {
            const auto& sel = chip_selects[cs];
            for (std::size_t c = 0; c < kNumChipsPerCS; ++c) {
                const auto& chip = sel.chips[c];
                ...
                for (std::size_t g = 0; g < chip.read_map.num_groups(); ++g) {
                    if (!chip.read_map.is_group_on[g]) continue;
                    for (auto kind : chip.read_map.group_data_types[g]) {
                        if (kind == CELL_TEMPERATURE) { ++out[flat]; }
                    }
                }
            }
        }
    ...
    constexpr std::array<std::size_t, kTotalChips> board_temps_per_chip() const {
        std::array<std::size_t, kTotalChips> out{};
        for (std::size_t cs = 0; cs < kNumChipSelects; ++cs) {
            const auto& sel = chip_selects[cs];
            for (std::size_t c = 0; c < kNumChipsPerCS; ++c) {
                ...
                for (std::size_t g = 0; g < chip.read_map.num_groups(); ++g) {
                    if (!chip.read_map.is_group_on[g]) continue;
                    for (auto kind : chip.read_map.group_data_types[g]) {
                        if (kind == BOARD_TEMPERATURE) { ++out[flat]; }
                    }
                }
            }
        }
    ...
    // ---- Derived totals (sum of per-chip) ----
    constexpr std::size_t num_voltage_cells_total() const {
        const auto v = voltage_cells_per_chip();
        std::size_t s = 0; for (auto x : v) s += x; return s;
    }

    static constexpr std::size_t to_global_index(std::size_t cs_index,
                                                 std::size_t chip_index) noexcept {
        return cs_index * kNumChipsPerCS + chip_index;
    }


    constexpr std::size_t num_temp_cells_total() const {
        const auto t = temp_cells_per_chip();
        std::size_t s = 0; for (auto x : t) s += x; return s;
    }

    constexpr std::size_t num_board_temps_total() const {
        const auto b = board_temps_per_chip();
        std::size_t s = 0; for (auto x : b) s += x; return s;
    }
};
#endif
